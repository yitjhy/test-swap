...
const RemoveSection: FC<TRemoveSection> = ({ data, onLiquidityChange }) => {
  ...
  const [changeTokens, setChangeTokens] = useState<Global.TPairDetail['tokens']>([])
  const [swapSectionTokens, setSwapSectionTokens] = useState<Global.TPairDetail['tokens']>([])
  ...
  const switchTokens = (value: string) => {
    const res = data.tokens?.map((item) => {
      return {
        ...item,
        balanceOfPair: item?.balanceOfPair
          ? item?.balanceOfPair.mul(parseUnits(value, 5)).div(parseUnits('1', 7))
          : constants.Zero,
      }
    })
    setChangeTokens(res)
  ...
  const handleSliderChange = (value: number) => {
    setPercentRate(value)
    switchTokens(String(value))
  }
  const onChangeType = () => {
    setRemoveType(removeType === RemoveType.simple ? RemoveType.detailed : RemoveType.simple)
  }
  const [liquidity, setLiquidity] = useState<BigNumber>(constants.Zero)
  ...
  useEffect(() => {
    if (data.tokens?.length) {
      const res = data.tokens.map((item) => {
        return {
          ...item,
          balance: item.balanceOfPair,
          // balanceOfPair: item.balanceOfPair.mul(parseUnits(value, 5)).div(parseUnits('1', 7)),
        }
      })
      switchTokens(String(percentRate))
      setSwapSectionTokens(res as Global.TPairDetail['tokens'])
    }
  }, [data.tokens])
    ...
    <RemoveSectionWrapper removeType={removeType}>
      ...
      <>
        {removeType === RemoveType.detailed ? (
          <>
            <div className="combination-currency-percent-rate">{percentRate}%</div>
            <SwapSection
              readonly={true}
              style={{ marginTop: 7 }}
          ...
          <>
            ...
            <Slider onChange={handleSliderChange} value={percentRate} />
            <div className="fast-btn-wrapper">
              <button
                onClick={() => {
                  setPercentRate(25)
                  switchTokens(String(25))
                }}
              >
                25%
              </button>
...
const RemoveSectionWrapper = styled.div<{ removeType: RemoveType }>`
  border: 1px solid #262626;
  padding: 1.3rem 0.75rem;
  row-gap: 1.2rem;
  margin-top: 20px;
  display: ${({ removeType }) => (removeType === RemoveType.simple ? 'grid' : 'block')};
  .title-wrapper {
    display: flex;
