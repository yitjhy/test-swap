...
export type TLPDetailProps = {
  accountPairBalance: BigNumber
  pairDecimals: number
  LPShare: BigNumber
  pairAddress: string
  rate: { rate: BigNumber; fromCurrency: TCurrencyListItem; toCurrency: TCurrencyListItem }[]
  tokens: [TCurrencyListItem, TCurrencyListItem]
}
const LPDetail: FC<{ data: TLPDetailProps }> = ({ data }) => {
  return (
    <LPShareWrapper>
      <div className="lp-share-item-wrapper">
        ...
        <span className="lp-share-value">{formatUnits(data.accountPairBalance, data.pairDecimals)}</span>
      </div>
      <div className="lp-share-item-wrapper">
        <span className="lp-share-label">Your LP Share：</span>
        <span className="lp-share-value">{formatUnits(data.LPShare, 6)}%</span>
      </div>
      <>
...
const LPShareWrapper = styled.div`
  display: grid;
  row-gap: 1.5rem;
  margin-top: 20px;
  .lp-share-item-wrapper {
    display: flex;
