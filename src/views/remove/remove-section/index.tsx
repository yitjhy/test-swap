...
type TRemoveSection = {
  tokens: TLPDetailProps['tokens']
}
const RemoveSection: FC<TRemoveSection> = ({ tokens }) => {
  ...
  const switchTokens = (value: string) => {
    const res = tokens.map((item) => {
      return {
        ...item,
        balance: item.balance.mul(parseUnits(value, 5)).div(parseUnits('1', 7)),
      }
    })
        ...
        {removeType === RemoveType.detailed ? (
          ...
          <>
            ...
            <div className="currency-amount-wrapper">
              {changeTokens.map((item, index) => {
                return (
                  <div className="currency-amount-item-wrapper" key={index}>
                    <span className="currency-amount">{formatUnits(item.balance, item.decimals)}</span>
              ...
              {/*  <span className="currency-amount">0.0000147</span>*/}
              {/*  <span className="currency-symbol-wrapper">*/}
              {/*    <Image className="currency-logo" src={currencyIcon} alt="" width={26} height={26} />*/}
              {/*    USDT*/}
              {/*  </span>*/}
